#include "carla_zww_pid_controller/vehicle_longitudinal_controller_pid.h"

#include <math.h>

#include <fstream>

#include "carla_zww_pid_controller/common.h"

using namespace std;

static const rclcpp::Logger LOGGER = rclcpp::get_logger("carla_zww_pid_controller_publisher");

// Controller
zww::control::PIDController yaw_pid_controller(0.5, 0.3, 0.1);             // 转向角pid
// zww::control::PIDController speed_pid_controller(0.206, 0.0206, 0.515);    // 速度pid Kp Ki Kd
zww::control::PIDController speed_pid_controller(0.16, 0.02, 0.01);    // 速度pid Kp Ki Kd

VehicleControlPublisher::VehicleControlPublisher()
    : Node("carla_zww_pid_controller")
/*'''**************************************************************************************
- FunctionName: None
- Function    : None
- Inputs      : None
...
**************************************************************************************'''*/
{
    ...
    // 读取参考线路径
    std::ifstream infile("src/carla_ros_bridge/carla_zww_project_1/carla_zww_pid_controller/data/gps_data_2022_09_09_15_18_45.csv", ios::in);    //将文件流对象与文件连接起来
