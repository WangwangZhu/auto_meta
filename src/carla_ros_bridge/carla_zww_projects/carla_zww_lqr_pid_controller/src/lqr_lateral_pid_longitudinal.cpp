#include "carla_zww_lqr_pid_controller/lqr_lateral_pid_longitudinal.h"
...
**************************************************************************************'''*/
{
    ...
    carla_status_subscriber = this->create_subscription<carla_msgs::msg::CarlaEgoVehicleStatus>("/carla/ego_vehicle/vehicle_status", 10, std::bind(&LQRControllerNode::VehicleStatusCallback, this, _1));
...
**************************************************************************************'''*/
{
    ...
    if (!firstRecord_) {    //有定位数据开始控制
        ...
        if (!isReachGoal_) {
            lqr_controller_lateral->ComputeControlCommand(this->vehicleState_, this->planning_published_trajectory, cmd);
        }
