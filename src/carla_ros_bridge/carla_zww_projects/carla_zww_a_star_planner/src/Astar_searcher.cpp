#include "carla_zww_a_star_planner/Astar_searcher.h"
...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    */
    ...
    // **** Manhattan *****
    // distance_heuristic = std::abs(node1_coordinate(0) - node2_coordinate(0)) + std::abs(node1_coordinate(1) - node2_coordinate(1)) + std::abs(node1_coordinate(2) - node2_coordinate(2));

    // **** Euclidean  *****
    // distance_heuristic = std::pow(node1_coordinate(0) - node2_coordinate(0), 2) + std::pow(node1_coordinate(1) - node2_coordinate(1), 2) + std::pow(node1_coordinate(2) - node2_coordinate(2), 2);

    // **** Diagonal  *****
    double cost_step = 1.0;
    double dx = std::abs(node1_coordinate(0) - node2_coordinate(0));
    double dy = std::abs(node1_coordinate(1) - node2_coordinate(1));
    double dz = std::abs(node1_coordinate(2) - node2_coordinate(2));
    distance_heuristic = cost_step * (dx + dy + dz) + (std::sqrt(cost_step * 3.0) - cost_step * 3.0) * std::min({dx, dy, dz});
