...
class zwwControl(CompatibleNode):
    """
    Handle input events
    """
    def __init__(self):
        super(zwwControl, self).__init__("zwwControl")
...
def main(args=None):
    ...
    main function
    """
    roscomp.init("zwwControl", args=args)

    try:
        zww_control_node = zwwControl()
        executor = roscomp.executors.MultiThreadedExecutor()
        executor.add_node(zww_control_node)
        update_timer = zww_control_node.new_timer(0.01, lambda timer_event=None: zww_control_node._on_new_carla_frame())
        target = zww_control_node.spin()
