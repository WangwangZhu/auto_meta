...
void StanleyController::LoadControlConf() {
    ...
    stanley_eps_ = 0.01;              // Stanley 迭代求解精度
    stanley_max_iteration_ = 1500;    // Stanley 的迭代次数

    return;
}

// 初始化控制器
void StanleyController::Init() {
    ...
    */
    ...
    // Stanley cost function中 输入值u的权重
    matrix_r_ = Matrix::Identity(1, 1);
    matrix_r_(0, 0) = 10;
    // Stanley cost function中 状态向量x的权重
...
// **TODO **计算控制命令 该函数定频调用
bool StanleyController::ComputeControlCommand(const VehicleState &localization, const TrajectoryData &planning_published_trajectory, ControlCmd &cmd) {
    // 规划轨迹
    ...
    */
    ...
    // TODO 05 Solve Stanley Problem
    SolveStanleyProblem(matrix_ad_, matrix_bd_, matrix_q_, matrix_r_, stanley_eps_, stanley_max_iteration_, &matrix_k_);
...
// 计算横向误差并且更新状态向量x
void StanleyController::UpdateState(const VehicleState &vehicle_state) {
...
// TODO 04 更新状态矩阵A并将状态矩阵A离散化
void StanleyController::UpdateMatrix(const VehicleState &vehicle_state) {
...
double StanleyController::ComputeFeedForward(const VehicleState &localization, double ref_curvature) {
...
// TODO 03 计算误差
void StanleyController::ComputeLateralErrors(const double x, const double y, const double theta, const double linear_v, const double angular_v, const double linear_a, LateralControlErrorPtr &lat_con_err) {
...
// TODO 05:求解 Stanley 方程
void StanleyController::SolveStanleyProblem(const Matrix &A, const Matrix &B, const Matrix &Q, const Matrix &R, const double tolerance, const uint max_num_iteration, Matrix *ptr_K) {
    // 防止矩阵的维数出错导致后续的运算失败
    if (A.rows() != A.cols() || B.rows() != A.rows() || Q.rows() != Q.cols() || Q.rows() != A.rows() || R.rows() != R.cols() || R.rows() != B.cols()) {
        std::cout << "Stanley solver: one or more matrices have incompatible dimensions." << std::endl;
        return;
    }
    Eigen::MatrixXd P = Q;
