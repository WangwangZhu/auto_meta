...
#include <algorithm>
#include <iomanip>
#include "fsm_decision_making/utils.h"
...
class FSMDecisionMaking : public rclcpp::Node
{
public:    
    FSMDecisionMaking();
    virtual ~FSMDecisionMaking();
...
public:
    ...
    rclcpp::Publisher<behavior_decision_interface::msg::FSMDecisionResults>::SharedPtr fsm_behavior_decision_makeing_publisher;
    behavior_decision_interface::msg::FSMDecisionResults behavior_decision_result_msg;
    ...
    visualization_msgs::msg::Marker fsm_decision_making_path_cardesian;
    geometry_msgs::msg::Point fsm_decision_making_point_cartesian;

    nav_msgs::msg::Path fsm_decision_making_path_frenet;
    geometry_msgs::msg::PoseStamped fsm_decision_making_point_frenet;
...
public:
    ...
    int fsm_decision_making_path_id = 10;
    ...
    // 2 向右变道 
    // 4 跟车行驶
    // 5 车道内微调，道路曲率很大，切边行驶，遇到卡车，弱势交通参与者，在车道内适当远离
    // 6 返回期望车道
    // 7 车辆起步
    // 8 定点停车
    // 9 自由巡航
